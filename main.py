...
trading_interval_seconds = 30  # Intervalo de análise do mercado (2 minutos)
...
def add_log(message: str, category: str = "info"):
    """Adiciona um log na lista em memória para exibição no frontend."""
    log_entry = {
...
def record_trade_deltas(old_eth: float, old_usdc: float, new_eth: float, new_usdc: float):
    ...
    if delta_eth > 0.00001 and delta_usdc < -0.01:
        ...
        # Obter o preço do mercado atual para verificar se o delta de ETH está poluído (ex: faucet)
    ...
    # VENDA de ETH: saldo de ETH diminuiu e USDC aumentou significativamente
        ...
        # Obter o preço do mercado atual para verificar se o delta de USDC está poluído
        market_price = fetch_eth_price()
        if market_price > 0.0 and abs(price - market_price) / market_price > 0.15:
            logger.warning(
                f"[Aviso] Preço calculado do swap (${price:.2f}) desvia mais de 15% do preço de mercado (${market_price:.2f}). "
...
async def sltp_monitoring_loop():
    ...
    while True:
        try:
            ...
            if targets:
                # Buscar preços atuais de mercado
                try:
                    ...
                    logger.error(f"Erro ao obter mids da Hyperliquid para monitoramento SL/TP: {e}")
                    mids = {}
                
                # Para evitar erros de mutação durante iteração
...
async def autonomous_trading_loop():
    ...
    while is_autonomous_running:
        try:
            ...
            # Consultar status de futuros da Hyperliquid
            ...
            # Prompt instruindo o agente a fazer análise e negociar seguindo a estratégia
            prompt = (
                ...
                f"   - Use 'calculate_technical_indicators' para analisar as condições técnicas. Se não tiver nenhuma posição de futuros aberta, você pode abrir uma nova posição de futuros em 'ETH' ou 'BTC'.\n"
                f"   - LONG (open_hyperliquid_position): Se o RSI estiver em sobrevenda extrema (<35). Defina alavancagem de 3x a 5x isolado, com Stop Loss em 5% e Take Profit em 10% (sl_percent=5.0, tp_percent=10.0).\n"
                f"   - SHORT (open_hyperliquid_position): Se o RSI estiver em sobrecompra extrema (>65). Defina alavancagem de 3x a 5x isolado, com Stop Loss em 5% e Take Profit em 10% (sl_percent=5.0, tp_percent=10.0).\n"
                f"   - FECHAMENTO ANTECIPADO: Se você tiver uma posição ativa e os indicadores indicarem exaustão extrema (ex: RSI > 75 para LONG ativo, ou RSI < 25 para SHORT ativo), use 'close_hyperliquid_position' para fechar e garantir lucro.\n"
                f"4. **PRESERVAR GÁS**: Nunca faça swaps de spot que reduzam seu saldo de ETH abaixo de 0.0005 ETH.\n"
                f"5. **HOLD (AGUARDAR)**: Se não houver oportunidade clara de trading spot/futuros, decida por APENAS AGUARDAR. Explique de forma analítica com base na tendência e indicadores fornecidos.\n\n"
            ...
            # Aguardar alguns segundos para a transação se propagar na blockchain
            await asyncio.sleep(5)

            # Consultar saldos depois e registrar trades
        ...
        # Dormir até o próximo intervalo de trading
...
async def get_performance():
    ...
    for label, days in windows.items():
        # Encontrar preço de início do período
        idx = -days - 1
        ...
        # 2. DCA Diário (Investindo $1 USD de ETH diariamente)
        dca_usd_spent = 0.0
...
async def get_futures_state():
    ...
    # Obter preços médios de mercado
    mids = {}
    try:
        from hyperliquid.info import Info
        from hyperliquid.utils import constants
    ...
    if dry_run:
        # Calcular PnL não realizado das posições simuladas com base nos mids reais
        ...
        # Salvar o estado simulado atualizado
        save_trading_state(state)
        ...
        try:
            ...
            # Limpar targets de posições que já foram fechadas externamente
...
async def post_chat(payload: ChatMessage):
    ...
    try:
        # Envia a mensagem do usuário ao agente ReAct
        # Executando invoke sincronamente (comum em LangChain / LangGraph simples)
        response = agent.invoke({"messages": [("user", user_msg)]})
        messages = response.get("messages", [])
        
        # Aguardar alguns segundos para a transação se propagar na blockchain Sepolia
        await asyncio.sleep(5)
        
        # Consultar saldos depois e registrar trades se houver diferença
        eth_after = fetch_eth_balance(wallet_address)
        usdc_after = fetch_usdc_balance(wallet_address)
        record_trade_deltas(eth_before, usdc_before, eth_after, usdc_after)
...
if __name__ == "__main__":
    import uvicorn
    # Inicializa o servidor FastAPI localmente na porta configurada
